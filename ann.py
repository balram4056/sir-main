...
cv_scores = cross_val_score(ann_model, X_train_vectors, y_train, cv=10, scoring='f1')

# Train the model on the full training set
ann_model.fit(X_train_vectors, y_train)

# Predict the labels of the test data
y_pred = ann_model.predict(X_test_vectors)
...
plt.title('10-Fold Cross-Validation F1 Scores for ANN')
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.ylim(0, 1)
plt.grid(True)
...
plt.title('Evaluation Metrics for ANN')
plt.ylabel('Score')
plt.ylim(0, 1)
plt.show()
